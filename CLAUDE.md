...
./build.sh debug --hyle             # + the hyle consensus engine (see "The Hyle layer")
./build.sh debug --hyle-src <path>  # ... against a local hyle checkout
...
## The Hyle layer (`ces.hyle`, `--hyle`)

An application-system built ON CES, not part of it: CES is the low-level service (carrier, identity, metering), hyle is a BFT chain riding it, and the K,V service rides hyle. Built only with `--hyle` (`--hyle-src <path>` to build against a local hyle checkout); `hyle_services` is statically linked into `cesluajitd`, so `ces.hyle` drives a `hyle::services::Runtime` directly, no IPC. Extension-only (`/s/`). Without `--hyle` the binding is absent and hylesolo says so.

`ces.hyle.*` is the CHAIN INTERFACE and says nothing about how many nodes are behind it: `submit(bytes)`, `sudo.propose/approve/pending/quorum` (+ `sudo.op.*` inner builders), `vote_add`/`vote_remove`, the reads (`info`, `config`, `account`, `entry`, `tx_result`, `validators`, `mint_key`, `self`), and `op.*` builders that sign AS THE PROGRAM. `ces.hyle.solo.*` is one implementation's LIFECYCLE (`start`/`tick`/`stop`): a single-validator, transportless chain driven one block per tick. A multi-node implementation adds its own lifecycle and leaves the interface alone, so a program written against `ces.hyle.*` moves to a real chain unchanged.

**The chain IS the K,V service.** A hyle `entry` is a mapping with an owner and a rent balance, so publish / update / give / delete / fund are `EntryOp{Put,Give,Del}` plus a transfer to `'e'+name`. Rent drains the entry per second of footprint; `Rip` is the permissionless cull (unsigned by construction: anyone reaps a starved entry and takes `rip_bounty`). CES does not invent any of it and does not parse it.

**Carrier principle.** `submit` takes opaque bytes. The client encodes and SIGNS its own op offline; the program relays bytes it cannot read and never holds a user's key. Ops the program signs as itself go through the same `submit` via `op.*` — it is just another client of its own service.

**One key, two ledgers.** CES and hyle both derive ed25519 from a 32-byte secret with CryptoPP, so `ces.hyle.self() == ces.program_pubkey()`, and any CES ed25519 identity is already a hyle account at the same 32 bytes. That is the bridge (a payment on one side can authorize a mint on the other with no mapping table) and the constraint: **signers must be ed25519** — a secp256k1 CES key has no hyle identity. Pinned by `HyleSoloTests/ACesIdentityIsAHyleIdentity`.

**Sudo** (hyle app-level, `hyle::services::SudoOp`) runs one act with the act's own guards waived — no signature, sequence, fee, or ownership check. Approval is collected ON CHAIN, EOSIO-msig-style: `Propose` opens a `'g'` cell (a new key prefix no user op can write, keyed by proposer) carrying the act plus the proposer's own vote; `Approve` adds a member vote bound to `sha256(inner)`; the vote that reaches quorum executes the act and erases the cell. A member holds at most one live proposal — that bounds the `'g'` namespace to the validator set, so the cell pays NO rent (unlike `'e'` entries); a stale proposal is overwritten by the proposer's next, or expires past `sudo_ttl_secs` (0 = never). Only the all-zero **mint sentinel** creates credit (a sudo transfer from it mints, counted in `sudo_minted`); every other sudo must find the money it moves. Sudo is NOT in hyle core: core only hands the app the active validator set + quorum via `ApplyContext` (it never decodes the act); the app owns authorization AND effect. The set is `validators_for(h)` — the `+2`-scheduled consensus set that committed the block, NOT the immediate governance membership `gov_.members()` — so sudo authority equals consensus authority even mid-membership-change; votes are recounted against the current set at execution, and a validator that leaves the set has its `'g'` cell reclaimed via the generic `StateMachine::on_validators_removed` hook (bound: one cell per current validator, zero orphans). A one-validator chain is its own quorum so `Propose` executes at once; the same path waits for `Approve`s on a bigger set; an identical re-`Propose` keeps its accumulated votes. Tests: `SudoTests` (hyle), incl. 4-member accumulation, recount-against-current-set, departed-proposer cell reclaim, re-propose-keeps-votes, non-member rejection, hash-committed votes, and TTL expiry.

`extensions/hylesolo.lua` is the reference implementor: the development net. It hosts the chain, autostarts it, and serves a line protocol over `ces.conn` (`info`/`config`/`self`/`account`/`entry`/`txr`/`submit`/`faucet`) reachable via `cesh dial <pid>` or any program. `faucet` sudo-mints to `conn.pubkey`, the key CES authenticated at bind, which is that caller's hyle account. Genesis invariants the chain enforces: `reward_base > fee_mint`, and with `rent_rate > 0`, `rip_bounty <= min(fee_transfer, fee_entry)` (else the cull is an inflation pump). Tests: `HyleSoloTests` (`tests/test_hyle.cpp`) — a real server, a real cesluajitd, a real chain, and a client that signs its own ops; `cestests` links `hyle_services` under `CES_HYLE`.

Client: `cesh hyle` (`src/cesh/cesh_hyle.inc`, `--hyle` build only) drives a chain-hosting instance over the `builtin:lua` line protocol, reusing the `cesh dial` bind+ATTACH (`DialLineSession` in `dial.cpp`). `hyle list` enumerates instances; every other verb names one by pid (`cesh hyle <pid> <verb>`), never guessing. Verbs: reads `info`/`config`/`self`/`height`/`mintkey`/`account`/`entry`/`txr` relay through; `get` emits an entry's value as raw bytes (`--out` file, else stdout, binary-safe); `put`/`del`/`give`/`transfer` build + sign an op with `hyle_services` and `submit` it; `faucet` mints; `submit` relays pre-signed op bytes (the carrier escape hatch). Value bytes come from `--in text:|hex:|file:` (shared with `cesh file put`). Only `cesh` links `hyle_services`; the CES client engine (`ceslib`) stays hyle-free. Governance is not a client verb: validators run servers and govern from the web-admin dashboard.
