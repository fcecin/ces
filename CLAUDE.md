...
- **Never reinvent wire serialization.** The canonical CES wire-bytes types and serialization API live in `<ces/buffer.h>`:
  ...
  **One exception**: `src/cesluajitd/main.cpp` is the LuaJIT-hosted compute child runtime, deliberately minimal — no ceslib link, no boost link, no CES/MINX/logkv include paths plumbed through CMake. It carries its own `put_u16`/`put_u64`/`get_u32`/etc. helpers internally. Documented inline at their definition site.
