...
// pair<const uint32_t, Alias> the alias store holds. The value is 138 bytes with
// zero padding; the entry is 144 bytes (4-byte id key + 138-byte value + 2 pad).
// Also round-trips every field through the accessors and the serializer.
...
constexpr std::size_t kEntryPad  = 2;   // pair rounds 142 up to the key's 4-byte align
constexpr std::size_t kSzEntry   = 144;
constexpr std::size_t kAlEntry   = 4;
} // namespace expected

static_assert(sizeof(ces::Alias) == expected::kSzAlias, "Alias value is 138 bytes");
...
static_assert(sizeof(AliasEntry) == expected::kSzEntry, "alias entry is 144 bytes");
static_assert(alignof(AliasEntry) == expected::kAlEntry);
static_assert(expected::kSumPayload == 138);
